...
public class JCthugha extends ParamNode implements Closeable {
	...
	/**
	 * Resets Wave and Bindings to the app's fresh-session defaults: a single enabled Oscilloscope
	 * instance plus a gentle rotation animation on it (matching the minimal, single-wave startup
	 * intent -- a fresh session shouldn't default to several overlapping wave types at once). Must
	 * be called after the full param tree is wired (i.e. after {@link
	...
	public void resetToDefaults() {
		...
		waveSystem.recreate(List.of(waveSpec(WaveSystem.WaveType.OSCILLOSCOPE)));
		OscilloscopeModel osc = (OscilloscopeModel) waveSystem.instances().get(0);
