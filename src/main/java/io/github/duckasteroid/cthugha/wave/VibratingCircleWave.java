...
public class VibratingCircleWave implements Wave {
  ...
  public void wave(AudioSample sound, ScreenBuffer buffer) {
    ...
    FrequencySpectra fft = sound.fft();
    if (fft != null) {
      double extent = fft.getMagnitude(frequency.value);
      ...
      if (fill.value) {
        graphics.fillOval(locationPoint.x - radius, locationPoint.y - radius, radius * 2,
          radius * 2);
      } else {
        graphics.drawOval(locationPoint.x - radius, locationPoint.y - radius, radius * 2,
          radius * 2);
      }
      graphics.dispose();
    }
