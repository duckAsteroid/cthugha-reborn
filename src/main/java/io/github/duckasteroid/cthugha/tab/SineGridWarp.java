 ...
 */
public class SineGridWarp extends AbstractNode implements TranslateTableSource {
  ...
  public PixelMapper generate(int width, int height, Random rng) {
    ...
    return (dstX, dstY, dst, dstOffset, r) -> {
      dst.put(dstOffset,     (short) TranslateTableSource.wrap((int)(dstX + sin(dstY * fx + px) * amp), width));
      dst.put(dstOffset + 1, (short) TranslateTableSource.wrap((int)(dstY + sin(dstX * fy + py) * amp), height));
    };
  }
}
